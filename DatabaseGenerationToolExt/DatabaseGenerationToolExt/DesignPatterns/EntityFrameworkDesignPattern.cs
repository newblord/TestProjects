	...
	public class EntityFrameworkDesignPattern : DesignPattern
	{
		...
		private void CreateRepositories()
		{
			...
			foreach (Table tbl in from t in Tables.Where(t => !t.IsMapping && t.TableData.GenerateRepository).OrderBy(x => x.NameHumanCase) select t)
			{
				...
				{
					WriteLine("public IEnumerable<{0}> GetBy{1}({2})", tbl.NameHumanCase, string.Join(string.Empty, index.Columns.Select(s => s.NameHumanCase)), index.CreateParameterString());
					OpenMethodBrace();
					WriteLine("return Context.{0}s.Where(x => {1} ).AsEnumerable();", tbl.NameHumanCase, index.CreateWhereString());
					CloseMethodBrace();
				}
				...
				{
					WriteLine("public {0} FindBy{1}({2})", tbl.NameHumanCase, string.Join(string.Empty, index.Columns.Select(s => s.NameHumanCase)), index.CreateParameterString());
					OpenMethodBrace();
			...
			foreach (Table tbl in from t in Tables.Where(t => !t.IsMapping && t.TableData.GenerateRepositoryInterface).OrderBy(x => x.NameHumanCase) select t)
			{
				...
				foreach (Index index in tbl.Indexes.Where(x => x.IsPrimaryKey == false && x.IsUnique == false).GroupBy(g => g.ColumnString).Select(s => s.First()))
				{
					WriteLine("IEnumerable<{0}> GetBy{1}({2});", tbl.NameHumanCase, string.Join(string.Empty, index.Columns.Select(s => s.NameHumanCase)), index.CreateParameterString());
				}

				foreach (Index index in tbl.Indexes.Where(x => x.IsPrimaryKey == false && x.IsUnique == true).GroupBy(g => g.ColumnString).Select(s => s.First()))
				{
					WriteLine("{0} FindBy{1}({2});", tbl.NameHumanCase, string.Join(string.Empty, index.Columns.Select(s => s.NameHumanCase)), index.CreateParameterString());
				}
