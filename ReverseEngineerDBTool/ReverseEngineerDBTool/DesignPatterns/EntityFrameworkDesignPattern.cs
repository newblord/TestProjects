...
namespace ReverseEngineerDBTool.DesignPatterns
{
    public class EntityFrameworkDesignPattern : DesignPattern
    {
        ...
        private void CreateModels()
        {
            ...
            foreach (Table tbl in from t in Tables.Where(t => !t.IsMapping).OrderBy(x => x.NameHumanCase) select t)
            {
                ...
                WriteLine($"using {ProjectSetting.EnumNamespace};");
                WriteLine("using Tiger.Extensions;");
                WriteLine("");
                ...
                foreach (TableColumn col in tbl.Columns.OrderBy(x => x.Ordinal).Where(x => !x.Hidden))
                {
                    ...
                    if (!string.IsNullOrEmpty(col.EnumDataTypeName))
                    {
                        WriteLine($"public {col.PropertyType} {col.Name}");
                        OpenBrace();
                        WriteLine("get");
                        OpenBrace();
                        WriteLine($"return {col.NameHumanCase}.EnumCode();");
                        CloseBrace();
                        WriteLine("set");
                        OpenBrace();
                        WriteLine($"{col.NameHumanCase} = value.ToEnum<{col.EnumDataTypeName}>();");
                        CloseBrace();
                        CloseBrace();

                        WriteLine("[NotMapped]");
                        WriteLine(col.EnumEntity);
                        WriteLine("");
                    }
                    else
                    {
            ...
            foreach (Table tbl in from t in Tables.Where(t => !t.IsMapping && t.TableData.GenerateModelInterface).OrderBy(x => x.NameHumanCase) select t)
            {
                ...
                foreach (TableColumn col in tbl.Columns.OrderBy(x => x.Ordinal).Where(x => !x.Hidden))
                {
                    if (!col.IsComputed)
                    {
                        if (!string.IsNullOrEmpty(col.InterfaceEntity))
                        {
            ...
            foreach (Table tbl in from t in Tables.Where(t => !t.IsMapping && t.TableData.GenerateModelDto).OrderBy(x => x.NameHumanCase) select t)
            {
                ...
                foreach (TableColumn col in tbl.Columns.OrderBy(x => x.Ordinal).Where(x => !x.Hidden))
                {
                    if (!string.IsNullOrEmpty(col.EnumDataTypeName))
                    {
                        WriteLine("public {0} {1} {{get; set;}}", col.EnumDataTypeName, col.NameHumanCase);
                    }
                    else
                    {
                        WriteLine(col.Entity);
        ...
        {
            foreach (StoredProcedure sp in StoredProcedures.Where(x => x.StoredProcData.StoredProcSelect && x.ReturnModels.Count > 0 && x.ReturnModels.Any(returnColumns => returnColumns.Any())))
            {
        ...
        private void CreateEnums()
        {
            ...
            {
                StartNewFile(@enum.Name + DatabaseSetting.GeneratedFileExtension, ProjectSetting.EnumProjectName, ProjectSetting.EnumFolderName);
                ...
                WriteLine("using Tiger.DataModel.Enums;");
                WriteLine("");

                BeginNamespace(ProjectSetting.EnumNamespace);
