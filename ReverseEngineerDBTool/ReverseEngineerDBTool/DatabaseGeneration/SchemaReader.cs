...
using ReverseEngineerDBTool.DatabaseGeneration.Models;

namespace ReverseEngineerDBTool.DatabaseGeneration
{
    public class SchemaReader
    {
        ...
        private const string ForeignKeySQL = @"
			...
			WHERE FK.name {TABLE_NAMES} AND PK.name {TABLE_NAMES}
        ...
        {
            string tableSql = string.Format("{0} ('{1}')", Global.SelectedTables.Count == 0 ? "NOT IN" : "IN", string.Join("', '", Global.SelectedTables.Select(x => x.TableName)));
            string sql = TableSQL.Replace("{TABLE_NAMES}", tableSql);
            return sql;
        }
        ...
        {
            string tableSql = string.Format("{0} ('{1}')", Global.SelectedTables.Count == 0 ? "NOT IN" : "IN", string.Join("', '", Global.SelectedTables.Select(x => x.TableName)));
            string sql = ForeignKeySQL.Replace("{TABLE_NAMES}", tableSql);
            return sql;
        }
        ...
        {
            string tableSql = string.Format("{0} ('{1}')", Global.SelectedTables.Count == 0 ? "NOT IN" : "IN", string.Join("', '", Global.SelectedTables.Select(x => x.TableName)));
            string sql = IndexSQL.Replace("{TABLE_NAMES}", tableSql);
            return sql;
        }

        private string ProcessStoredProcSQL()
        {
            string storedProcSql = string.Format("IN ('{0}')", string.Join("', '", Global.SelectedStoredProcedures.Select(x => x.StoredProcName)));
            string sql = StoredProcedureSQL.Replace("{SPROC_NAMES}", storedProcSql);
            return sql;
        }
        ...
        /// };
        /// </summary>
        private string StoredProcedureRename(string name, string schema)
        {
        ...
        public Tables LoadTables(bool loadAllTables)
        {
            if (Factory == null)
                return new Tables();
            ...
            try
            {
                using (DbConnection conn = Factory.CreateConnection())
                {
                    ...
                    var tables = ReadTables(loadAllTables);
                    tables.SetupConfig();
        ...
        public List<Models.Enum> LoadEnums()
        {
            ...
            try
            {
                ...
                foreach (Forms.Models.SpellFunctionData selectedEnum in Global.SelectedEnums)
                {
                    ...
                    newEnum.SpellFunctionData = selectedEnum;
                    newEnum.Name = GetEnumerationNameFromSpellFunction(selectedEnum.FunctionName);
        ...
        public Tables ReadTables(bool loadAllTables)
        {
            ...
            if (loadAllTables)
            {
                Cmd.CommandText = TableSQL.Replace("{TABLE_NAMES}", "NOT IN ('')");
            }
            else
            {
                ...
                while (rdr.Read())
                {
                    ...
                    if (lastTable != tableName || table == null)
                    {
                        ...
                        if (table == null)
                        {
                            ...
                            var tableData = Global.SelectedTables.Where(x => x.TableName.Equals(tableName, StringComparison.CurrentCultureIgnoreCase)).FirstOrDefault();

                            table.TableData = tableData ?? new Forms.Models.TableData();
            ...
            try
            {
                using (DbDataReader rdr = Cmd.ExecuteReader())
                {
                    while (rdr.Read())
                    {
                        ...
                        Index index = indexes.Where(x => x.TableName == table.Name && x.IndexName == rdr["IndexName"] as string).FirstOrDefault();
        ...
        private TableColumn CreateColumn(IDataRecord rdr, Regex rxClean, Table table)
        {
            ...
            {
                col.EnumDataTypeName = (string)table.TableData.Columns.Where(x => x.Name == col.NameHumanCase).First().Value;
            }
