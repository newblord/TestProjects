        ...
        private void ProcessSqlNode(Node contentElement, TSqlStandardFormattingState state)
        {
            ...
            switch (contentElement.Name)
            {
                ...
                case SqlStructureConstants.MERGE_ACTION:
                    ...
                    foreach (Node statement in contentElement.ChildrenByName(SqlStructureConstants.SQL_STATEMENT))
                        foreach (Node clause in statement.ChildrenByName(SqlStructureConstants.SQL_CLAUSE))
                            foreach (Node ifStatement in clause.ChildrenByName(SqlStructureConstants.IF_STATEMENT))
                                singleStatementIsIf = true;

					if (singleStatementIsIf && contentElement.Parent.Name.Equals(SqlStructureConstants.ELSE_CLAUSE))
					{
                ...
                case SqlStructureConstants.JOIN_ON_SECTION:
                    if (Options.BreakJoinOnSections)
                        state.BreakExpected = true;
                    ProcessSqlNodeList(contentElement.ChildrenByName(SqlStructureConstants.CONTAINER_OPEN), state);
                    if (Options.BreakJoinOnSections)
                        state.IncrementIndent();
                    ProcessSqlNodeList(contentElement.ChildrenByName(SqlStructureConstants.CONTAINER_GENERALCONTENT), state);
                    if (Options.BreakJoinOnSections)
                        state.DecrementIndent();
                    break;
                ...
                case SqlStructureConstants.ELSE_CLAUSE:
                    ProcessSqlNodeList(contentElement.ChildrenByName(SqlStructureConstants.CONTAINER_OPEN), state.DecrementIndent());
                    ProcessSqlNodeList(contentElement.ChildrenByName(SqlStructureConstants.CONTAINER_SINGLESTATEMENT), state.IncrementIndent());
                    break;
                ...
                case SqlStructureConstants.CTE_AS_BLOCK:
                    state.BreakExpected = true;
                    ProcessSqlNodeList(contentElement.ChildrenByName(SqlStructureConstants.CONTAINER_OPEN), state.DecrementIndent());
                    ProcessSqlNodeList(contentElement.ChildrenByName(SqlStructureConstants.CONTAINER_GENERALCONTENT), state.IncrementIndent());
                    break;
                ...
                case SqlStructureConstants.BETWEEN_CONDITION:
                    ProcessSqlNodeList(contentElement.ChildrenByName(SqlStructureConstants.CONTAINER_OPEN), state);
                    state.IncrementIndent();
                    ProcessSqlNodeList(contentElement.ChildrenByName(SqlStructureConstants.BETWEEN_LOWERBOUND), state.IncrementIndent());
                    if (Options.ExpandBetweenConditions)
                        state.BreakExpected = true;
                    ProcessSqlNodeList(contentElement.ChildrenByName(SqlStructureConstants.CONTAINER_CLOSE), state.DecrementIndent());
                    ProcessSqlNodeList(contentElement.ChildrenByName(SqlStructureConstants.BETWEEN_UPPERBOUND), state.IncrementIndent());
                    state.DecrementIndent();
                    state.DecrementIndent();
                    break;
				...
				case SqlStructureConstants.IN_PARENS:
					WhiteSpace_SeparateWords(state);
					if (contentElement.Name.Equals(SqlStructureConstants.EXPRESSION_PARENS) || contentElement.Name.Equals(SqlStructureConstants.IN_PARENS))
                        state.IncrementIndent();
                    state.AddOutputContent(FormatOperator("("), SqlHtmlConstants.CLASS_OPERATOR);
                ...
                case SqlStructureConstants.CATCH_BLOCK:
                    if (contentElement.Parent.Name.Equals(SqlStructureConstants.SQL_CLAUSE)
                        && contentElement.Parent.Parent.Name.Equals(SqlStructureConstants.SQL_STATEMENT)
                        && contentElement.Parent.Parent.Parent.Name.Equals(SqlStructureConstants.CONTAINER_SINGLESTATEMENT)
                        )
                        state.DecrementIndent();
                    ProcessSqlNodeList(contentElement.ChildrenByName(SqlStructureConstants.CONTAINER_OPEN), state);
                    ProcessSqlNodeList(contentElement.ChildrenByName(SqlStructureConstants.CONTAINER_MULTISTATEMENT), state);
                    state.DecrementIndent();
                    state.BreakExpected = true;
                    ProcessSqlNodeList(contentElement.ChildrenByName(SqlStructureConstants.CONTAINER_CLOSE), state);
                    state.IncrementIndent();
                    if (contentElement.Parent.Name.Equals(SqlStructureConstants.SQL_CLAUSE)
                        && contentElement.Parent.Parent.Name.Equals(SqlStructureConstants.SQL_STATEMENT)
                        && contentElement.Parent.Parent.Parent.Name.Equals(SqlStructureConstants.CONTAINER_SINGLESTATEMENT)
                ...
                case SqlStructureConstants.CASE_STATEMENT:
                    ProcessSqlNodeList(contentElement.ChildrenByName(SqlStructureConstants.CONTAINER_OPEN), state);
                    state.IncrementIndent();
                    ProcessSqlNodeList(contentElement.ChildrenByName(SqlStructureConstants.CASE_INPUT), state);
                    ProcessSqlNodeList(contentElement.ChildrenByName(SqlStructureConstants.CASE_WHEN), state);
                    ProcessSqlNodeList(contentElement.ChildrenByName(SqlStructureConstants.CASE_ELSE), state);
                ...
                case SqlStructureConstants.CASE_ELSE:
                    ...
                    ProcessSqlNodeList(contentElement.ChildrenByName(SqlStructureConstants.CONTAINER_OPEN), state);
                    ProcessSqlNodeList(contentElement.ChildrenByName(SqlStructureConstants.CONTAINER_GENERALCONTENT), state.IncrementIndent());
                    ProcessSqlNodeList(contentElement.ChildrenByName(SqlStructureConstants.CASE_THEN), state);
                    state.DecrementIndent();
                    break;
                ...
                case SqlStructureConstants.OR_OPERATOR:
                    if (Options.ExpandBooleanExpressions)
                        state.BreakExpected = true;
                    ProcessSqlNode(contentElement.ChildByName(SqlStructureConstants.OTHERKEYWORD), state);
                    break;
                ...
                case SqlStructureConstants.COMMENT_MULTILINE:
                    ...
                    if (contentElement.Parent.Name.Equals(SqlStructureConstants.SQL_STATEMENT)
                        || (contentElement.NextSibling() != null
                            && contentElement.NextSibling().Name.Equals(SqlStructureConstants.WHITESPACE)
                ...
                case SqlStructureConstants.COMMENT_SINGLELINE_CSTYLE:
                    ...
                    WhiteSpace_SeparateComment(contentElement, state);
                    state.AddOutputContent((contentElement.Name == SqlStructureConstants.COMMENT_SINGLELINE ? "--" : "//") + contentElement.TextValue.Replace("\r", "").Replace("\n", ""), SqlHtmlConstants.CLASS_COMMENT);
                    state.BreakExpected = true;
                ...
                case SqlStructureConstants.NSTRING:
                    ...
                    if (contentElement.Name.Equals(SqlStructureConstants.NSTRING))
                ...
                case SqlStructureConstants.COMMA:
                    ...
                    if (Options.TrailingCommas)
                    {
                        ...
                        if ((Options.ExpandCommaLists
								&& !(contentElement.Parent.Name.Equals(SqlStructureConstants.DDLDETAIL_PARENS)
									|| contentElement.Parent.Name.Equals(SqlStructureConstants.FUNCTION_PARENS)
									|| contentElement.Parent.Name.Equals(SqlStructureConstants.IN_PARENS)
									)
								)
        ...
        private void WhiteSpace_SeparateStatements(Node contentElement, TSqlStandardFormattingState state)
        {
            if (state.StatementBreakExpected)
            {
				...
				if (!(thisClauseStarter != null
					&& thisClauseStarter.Name.Equals(SqlStructureConstants.OTHERKEYWORD)
