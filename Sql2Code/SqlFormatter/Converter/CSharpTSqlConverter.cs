...
namespace SqlFormatter.Converter
{
    public class CSharpTSqlConverter
    {
        ...
        public CSharpTSqlConverter()
        {

        }

        public void ConvertTSqlToCode(ITokenList tokenList)
        {
            ParseTree sqlTree = new ParseTree(SqlStructureConstants.SQL_ROOT);
                ...
                switch (token.Type)
                {
                    case SqlTokenType.OpenParens:
                        ...
                        bool isInsertOrValuesClause = (
                            ...
                            && (
                                (firstNonCommentParensSibling.Name.Equals(SqlStructureConstants.OTHERKEYWORD)
                                   && firstNonCommentParensSibling.TextValue.ToUpperInvariant().StartsWith("INSERT")
                                   )
                                ||
                                (firstNonCommentParensSibling.Name.Equals(SqlStructureConstants.COMPOUNDKEYWORD)
                        ...
                        if (sqlTree.CurrentContainer.Name.Equals(SqlStructureConstants.CTE_ALIAS)
                            && sqlTree.CurrentContainer.Parent.Name.Equals(SqlStructureConstants.CTE_WITH_CLAUSE)
                            )
                            sqlTree.CurrentContainer = sqlTree.SaveNewElement(SqlStructureConstants.DDL_PARENS, "");
                        else if (sqlTree.CurrentContainer.Name.Equals(SqlStructureConstants.CONTAINER_GENERALCONTENT)
                            && sqlTree.CurrentContainer.Parent.Name.Equals(SqlStructureConstants.CTE_AS_BLOCK)
                            )
                            sqlTree.CurrentContainer = sqlTree.SaveNewElement(SqlStructureConstants.SELECTIONTARGET_PARENS, "");
                        else if (firstNonCommentParensSibling == null
                            && sqlTree.CurrentContainer.Name.Equals(SqlStructureConstants.SELECTIONTARGET)
                            )
                            sqlTree.CurrentContainer = sqlTree.SaveNewElement(SqlStructureConstants.SELECTIONTARGET_PARENS, "");
                        else if (firstNonCommentParensSibling != null
                            && firstNonCommentParensSibling.Name.Equals(SqlStructureConstants.SET_OPERATOR_CLAUSE)
                            )
                        {
                            ...
                            sqlTree.CurrentContainer = sqlTree.SaveNewElement(SqlStructureConstants.DDLDETAIL_PARENS, "");
                        else if (sqlTree.CurrentContainer.Name.Equals(SqlStructureConstants.DDL_PROCEDURAL_BLOCK)
                            || sqlTree.CurrentContainer.Name.Equals(SqlStructureConstants.DDL_OTHER_BLOCK)
                            || sqlTree.CurrentContainer.Name.Equals(SqlStructureConstants.DDL_DECLARE_BLOCK)
                            || (sqlTree.CurrentContainer.Name.Equals(SqlStructureConstants.SQL_CLAUSE)
                                && (firstNonCommentParensSibling != null
                                    && firstNonCommentParensSibling.Name.Equals(SqlStructureConstants.OTHERKEYWORD)
                            ...
                            sqlTree.CurrentContainer = sqlTree.SaveNewElement(SqlStructureConstants.DDL_PARENS, "");
                                    ...
                                    && lastNonCommentParensSibling.Name.Equals(SqlStructureConstants.ALPHAOPERATOR)
                                    && lastNonCommentParensSibling.TextValue.ToUpperInvariant().Equals("IN")
                                    )
                            )
                            sqlTree.CurrentContainer = sqlTree.SaveNewElement(SqlStructureConstants.IN_PARENS, "");
                        else if (IsLatestTokenAMiscName(sqlTree))
                            sqlTree.CurrentContainer = sqlTree.SaveNewElement(SqlStructureConstants.FUNCTION_PARENS, "");
                        else
                            sqlTree.CurrentContainer = sqlTree.SaveNewElement(SqlStructureConstants.EXPRESSION_PARENS, "");
                        break;
                    ...
                    case SqlTokenType.CloseParens:
                        ...
                        if (sqlTree.CurrentContainer.Name.Equals(SqlStructureConstants.DDLDETAIL_PARENS)
                            || sqlTree.CurrentContainer.Name.Equals(SqlStructureConstants.DDL_PARENS)
                            || sqlTree.CurrentContainer.Name.Equals(SqlStructureConstants.FUNCTION_PARENS)
                            || sqlTree.CurrentContainer.Name.Equals(SqlStructureConstants.IN_PARENS)
                            || sqlTree.CurrentContainer.Name.Equals(SqlStructureConstants.EXPRESSION_PARENS)
                            || sqlTree.CurrentContainer.Name.Equals(SqlStructureConstants.SELECTIONTARGET_PARENS)
                            )
                        {
                            sqlTree.MoveToAncestorContainer(1); //unspecified parent node...
                        }
                        else if (sqlTree.CurrentContainer.Name.Equals(SqlStructureConstants.SQL_CLAUSE)
                                && sqlTree.CurrentContainer.Parent.Name.Equals(SqlStructureConstants.SELECTIONTARGET_PARENS)
                                && sqlTree.CurrentContainer.Parent.Parent.Name.Equals(SqlStructureConstants.CONTAINER_GENERALCONTENT)
                                && sqlTree.CurrentContainer.Parent.Parent.Parent.Name.Equals(SqlStructureConstants.CTE_AS_BLOCK)
                                )
                        {
                            sqlTree.MoveToAncestorContainer(4, SqlStructureConstants.CTE_WITH_CLAUSE);
                            sqlTree.CurrentContainer = sqlTree.SaveNewElement(SqlStructureConstants.CONTAINER_GENERALCONTENT, "");
                        }
                        else if (sqlTree.CurrentContainer.Name.Equals(SqlStructureConstants.SQL_CLAUSE)
                                && (
                                    sqlTree.CurrentContainer.Parent.Name.Equals(SqlStructureConstants.EXPRESSION_PARENS)
                                    || sqlTree.CurrentContainer.Parent.Name.Equals(SqlStructureConstants.IN_PARENS)
                                    || sqlTree.CurrentContainer.Parent.Name.Equals(SqlStructureConstants.SELECTIONTARGET_PARENS)
                                )
                            )
                        {
                        ...
                        {
                            sqlTree.SaveNewElementWithError(SqlStructureConstants.OTHERNODE, ")");
                        }
                        break;
                    ...
                    case SqlTokenType.OtherNode:
                        ...
                        if (sqlTree.PathNameMatches(0, SqlStructureConstants.PERMISSIONS_DETAIL))
                        {
                            ...
                            {
                                sqlTree.MoveToAncestorContainer(1, SqlStructureConstants.PERMISSIONS_BLOCK);
                                sqlTree.StartNewContainer(SqlStructureConstants.PERMISSIONS_TARGET, token.Value, SqlStructureConstants.CONTAINER_GENERALCONTENT);
                            }
                            ...
                            {
                                sqlTree.MoveToAncestorContainer(1, SqlStructureConstants.PERMISSIONS_BLOCK);
                                sqlTree.StartNewContainer(SqlStructureConstants.PERMISSIONS_RECIPIENT, token.Value, SqlStructureConstants.CONTAINER_GENERALCONTENT);
                            }
                            else
                            {
                                //default to "some classification of permission"
                                sqlTree.SaveNewElement(SqlStructureConstants.OTHERKEYWORD, token.Value);
                            }
                        }
                            ...
                            )
                        {
                            ...
                            sqlTree.CurrentContainer = sqlTree.SaveNewElement(SqlStructureConstants.DDL_PROCEDURAL_BLOCK, "");
                            sqlTree.SaveNewElement(SqlStructureConstants.OTHERKEYWORD, token.Value);
                        }
                        else if (_CursorDetector.IsMatch(significantTokensString))
                        {
                            sqlTree.ConsiderStartingNewStatement();
                            sqlTree.CurrentContainer = sqlTree.SaveNewElement(SqlStructureConstants.CURSOR_DECLARATION, "");
                            sqlTree.SaveNewElement(SqlStructureConstants.OTHERKEYWORD, token.Value);
                        }
                        else if (sqlTree.PathNameMatches(0, SqlStructureConstants.DDL_PROCEDURAL_BLOCK)
                            ...
                            )
                        {
                            ...
                            sqlTree.CurrentContainer = sqlTree.SaveNewElement(SqlStructureConstants.TRIGGER_CONDITION, "");
                            Node triggerConditionType = sqlTree.SaveNewElement(SqlStructureConstants.COMPOUNDKEYWORD, "");
                            ...
                            AppendNodesWithMapping(sqlTree, tokenList.GetRangeByIndex(significantTokenPositions[0], significantTokenPositions[triggerConditionTypeNodeCount - 1]), SqlStructureConstants.OTHERKEYWORD, triggerConditionType);
                            ...
                            AppendNodesWithMapping(sqlTree, tokenList.GetRangeByIndex(significantTokenPositions[triggerConditionTypeNodeCount - 1] + 1, significantTokenPositions[triggerConditionTypeNodeCount + triggerConditionNodeCount - 1]), SqlStructureConstants.OTHERKEYWORD, sqlTree.CurrentContainer);
                            tokenID = significantTokenPositions[triggerConditionTypeNodeCount + triggerConditionNodeCount - 1];
                            sqlTree.MoveToAncestorContainer(1, SqlStructureConstants.DDL_PROCEDURAL_BLOCK);
                        }
                        else if (significantTokensString.StartsWith("FOR "))
                        {
                            ...
                            if (sqlTree.PathNameMatches(0, SqlStructureConstants.CURSOR_DECLARATION))
                            {
                                sqlTree.StartNewContainer(SqlStructureConstants.CURSOR_FOR_BLOCK, token.Value, SqlStructureConstants.CONTAINER_GENERALCONTENT);
                                sqlTree.StartNewStatement();
                            }
                            else if (sqlTree.PathNameMatches(0, SqlStructureConstants.SQL_CLAUSE)
                                && sqlTree.PathNameMatches(1, SqlStructureConstants.SQL_STATEMENT)
                                && sqlTree.PathNameMatches(2, SqlStructureConstants.CONTAINER_GENERALCONTENT)
                                && sqlTree.PathNameMatches(3, SqlStructureConstants.CURSOR_FOR_BLOCK)
                                )
                            {
                                sqlTree.MoveToAncestorContainer(4, SqlStructureConstants.CURSOR_DECLARATION);
                                sqlTree.StartNewContainer(SqlStructureConstants.CURSOR_FOR_OPTIONS, token.Value, SqlStructureConstants.CONTAINER_GENERALCONTENT);
                            }
                            else
                            {
                        ...
                        {
                            sqlTree.ConsiderStartingNewStatement();
                            sqlTree.CurrentContainer = sqlTree.SaveNewElement(SqlStructureConstants.DDL_DECLARE_BLOCK, "");
                            sqlTree.SaveNewElement(SqlStructureConstants.OTHERKEYWORD, token.Value);
                        }
                        ...
                        {
                            sqlTree.ConsiderStartingNewStatement();
                            sqlTree.CurrentContainer = sqlTree.SaveNewElement(SqlStructureConstants.DDL_OTHER_BLOCK, "");
                            sqlTree.SaveNewElement(SqlStructureConstants.OTHERKEYWORD, token.Value);
                        }
                            ...
                            )
                        {
                            if (significantTokensString.StartsWith("GRANT ")
                                && sqlTree.PathNameMatches(0, SqlStructureConstants.CONTAINER_GENERALCONTENT)
                                && sqlTree.PathNameMatches(1, SqlStructureConstants.DDL_WITH_CLAUSE)
                                && sqlTree.PathNameMatches(2, SqlStructureConstants.PERMISSIONS_BLOCK)
                            ...
                            {
                                sqlTree.ConsiderStartingNewStatement();
                                sqlTree.StartNewContainer(SqlStructureConstants.PERMISSIONS_BLOCK, token.Value, SqlStructureConstants.PERMISSIONS_DETAIL);
                            }
                        }
                        else if (sqlTree.CurrentContainer.Name.Equals(SqlStructureConstants.DDL_PROCEDURAL_BLOCK)
                        ...
                        {
                            sqlTree.SaveNewElement(SqlStructureConstants.OTHERKEYWORD, token.Value, sqlTree.SaveNewElement(SqlStructureConstants.DDL_RETURNS, ""));
                        }
                        else if (significantTokensString.StartsWith("AS "))
                        {
                            if (sqlTree.PathNameMatches(0, SqlStructureConstants.DDL_PROCEDURAL_BLOCK))
                            {
                                ...
                                else
                                {
                                    ...
                                    sqlTree.StartNewContainer(SqlStructureConstants.DDL_AS_BLOCK, token.Value, SqlStructureConstants.CONTAINER_GENERALCONTENT);
                                    sqlTree.StartNewStatement();
                                }
                            }
                            else if (sqlTree.PathNameMatches(0, SqlStructureConstants.CONTAINER_GENERALCONTENT)
                                && sqlTree.PathNameMatches(1, SqlStructureConstants.DDL_WITH_CLAUSE)
                                && sqlTree.PathNameMatches(2, SqlStructureConstants.DDL_PROCEDURAL_BLOCK)
                                )
                            {
                                sqlTree.MoveToAncestorContainer(2, SqlStructureConstants.DDL_PROCEDURAL_BLOCK);
                                sqlTree.StartNewContainer(SqlStructureConstants.DDL_AS_BLOCK, token.Value, SqlStructureConstants.CONTAINER_GENERALCONTENT);
                                sqlTree.StartNewStatement();
                            }
                            else if (sqlTree.PathNameMatches(0, SqlStructureConstants.CTE_ALIAS)
                                && sqlTree.PathNameMatches(1, SqlStructureConstants.CTE_WITH_CLAUSE)
                                )
                            {
                                sqlTree.MoveToAncestorContainer(1, SqlStructureConstants.CTE_WITH_CLAUSE);
                                sqlTree.StartNewContainer(SqlStructureConstants.CTE_AS_BLOCK, token.Value, SqlStructureConstants.CONTAINER_GENERALCONTENT);
                            }
                            else
                            {
                                sqlTree.SaveNewElement(SqlStructureConstants.OTHERKEYWORD, token.Value);
                        ...
                        {
                            sqlTree.ConsiderStartingNewStatement();
                            ProcessCompoundKeyword(tokenList, sqlTree, sqlTree.SaveNewElement(SqlStructureConstants.BEGIN_TRANSACTION, ""), ref tokenID, significantTokenPositions, 3);
                        }
                        ...
                        {
                            sqlTree.ConsiderStartingNewStatement();
                            ProcessCompoundKeyword(tokenList, sqlTree, sqlTree.SaveNewElement(SqlStructureConstants.BEGIN_TRANSACTION, ""), ref tokenID, significantTokenPositions, 2);
                        }
                        ...
                        {
                            sqlTree.ConsiderStartingNewStatement();
                            ProcessCompoundKeyword(tokenList, sqlTree, sqlTree.SaveNewElement(SqlStructureConstants.SAVE_TRANSACTION, ""), ref tokenID, significantTokenPositions, 2);
                        }
                        ...
                        {
                            sqlTree.ConsiderStartingNewStatement();
                            ProcessCompoundKeyword(tokenList, sqlTree, sqlTree.SaveNewElement(SqlStructureConstants.COMMIT_TRANSACTION, ""), ref tokenID, significantTokenPositions, 2);
                        }
                        else if (significantTokensString.StartsWith("COMMIT "))
                        {
                            sqlTree.ConsiderStartingNewStatement();
                            sqlTree.SaveNewElement(SqlStructureConstants.OTHERKEYWORD, token.Value, sqlTree.SaveNewElement(SqlStructureConstants.COMMIT_TRANSACTION, token.Value));
                        }
                        ...
                        {
                            sqlTree.ConsiderStartingNewStatement();
                            ProcessCompoundKeyword(tokenList, sqlTree, sqlTree.SaveNewElement(SqlStructureConstants.ROLLBACK_TRANSACTION, ""), ref tokenID, significantTokenPositions, 2);
                        }
                        else if (significantTokensString.StartsWith("ROLLBACK "))
                        {
                            sqlTree.ConsiderStartingNewStatement();
                            sqlTree.SaveNewElement(SqlStructureConstants.OTHERKEYWORD, token.Value, sqlTree.SaveNewElement(SqlStructureConstants.ROLLBACK_TRANSACTION, token.Value));
                        }
                        else if (significantTokensString.StartsWith("BEGIN TRY "))
                        {
                            sqlTree.ConsiderStartingNewStatement();
                            Node newTryBlock = sqlTree.SaveNewElement(SqlStructureConstants.TRY_BLOCK, "");
                            Node tryContainerOpen = sqlTree.SaveNewElement(SqlStructureConstants.CONTAINER_OPEN, "", newTryBlock);
                            ProcessCompoundKeyword(tokenList, sqlTree, tryContainerOpen, ref tokenID, significantTokenPositions, 2);
                            Node tryMultiContainer = sqlTree.SaveNewElement(SqlStructureConstants.CONTAINER_MULTISTATEMENT, "", newTryBlock);
                            sqlTree.StartNewStatement(tryMultiContainer);
                        }
                        else if (significantTokensString.StartsWith("BEGIN CATCH "))
                        {
                            sqlTree.ConsiderStartingNewStatement();
                            Node newCatchBlock = sqlTree.SaveNewElement(SqlStructureConstants.CATCH_BLOCK, "");
                            Node catchContainerOpen = sqlTree.SaveNewElement(SqlStructureConstants.CONTAINER_OPEN, "", newCatchBlock);
                            ProcessCompoundKeyword(tokenList, sqlTree, catchContainerOpen, ref tokenID, significantTokenPositions, 2);
                            Node catchMultiContainer = sqlTree.SaveNewElement(SqlStructureConstants.CONTAINER_MULTISTATEMENT, "", newCatchBlock);
                            sqlTree.StartNewStatement(catchMultiContainer);
                        }
                        else if (significantTokensString.StartsWith("BEGIN "))
                        {
                            sqlTree.ConsiderStartingNewStatement();
                            sqlTree.StartNewContainer(SqlStructureConstants.BEGIN_END_BLOCK, token.Value, SqlStructureConstants.CONTAINER_MULTISTATEMENT);
                            sqlTree.StartNewStatement();
                        }
                        else if (significantTokensString.StartsWith("MERGE "))
                        {
                            ...
                            sqlTree.ConsiderStartingNewStatement();
                            sqlTree.ConsiderStartingNewClause();
                            sqlTree.StartNewContainer(SqlStructureConstants.MERGE_CLAUSE, token.Value, SqlStructureConstants.MERGE_TARGET);
                        }
                        else if (significantTokensString.StartsWith("USING "))
                        {
                            if (sqlTree.PathNameMatches(0, SqlStructureConstants.MERGE_TARGET))
                            {
                                sqlTree.MoveToAncestorContainer(1, SqlStructureConstants.MERGE_CLAUSE);
                                sqlTree.StartNewContainer(SqlStructureConstants.MERGE_USING, token.Value, SqlStructureConstants.SELECTIONTARGET);
                            }
                            else
                                sqlTree.SaveNewElementWithError(SqlStructureConstants.OTHERNODE, token.Value);
                        }
                        else if (significantTokensString.StartsWith("ON "))
                        {
                            sqlTree.EscapeAnySelectionTarget();

                            if (sqlTree.PathNameMatches(0, SqlStructureConstants.MERGE_USING))
                            {
                                sqlTree.MoveToAncestorContainer(1, SqlStructureConstants.MERGE_CLAUSE);
                                sqlTree.StartNewContainer(SqlStructureConstants.MERGE_CONDITION, token.Value, SqlStructureConstants.CONTAINER_GENERALCONTENT);
                            }
                            else if (!sqlTree.PathNameMatches(0, SqlStructureConstants.DDL_PROCEDURAL_BLOCK)
                                && !sqlTree.PathNameMatches(0, SqlStructureConstants.DDL_OTHER_BLOCK)
                                && !sqlTree.PathNameMatches(1, SqlStructureConstants.DDL_WITH_CLAUSE)
                                && !sqlTree.PathNameMatches(0, SqlStructureConstants.EXPRESSION_PARENS)
                                && !ContentStartsWithKeyword(sqlTree.CurrentContainer, "SET")
                                )
                            {
                                sqlTree.StartNewContainer(SqlStructureConstants.JOIN_ON_SECTION, token.Value, SqlStructureConstants.CONTAINER_GENERALCONTENT);
                            }
                            else
                            {
                                sqlTree.SaveNewElement(SqlStructureConstants.OTHERKEYWORD, token.Value);
                        ...
                        {
                            sqlTree.StartNewContainer(SqlStructureConstants.CASE_STATEMENT, token.Value, SqlStructureConstants.CASE_INPUT);
                        }
                        else if (significantTokensString.StartsWith("WHEN "))
                        {
                            sqlTree.EscapeMergeAction();

                            if (sqlTree.PathNameMatches(0, SqlStructureConstants.CASE_INPUT)
                                || (sqlTree.PathNameMatches(0, SqlStructureConstants.CONTAINER_GENERALCONTENT)
                                    && sqlTree.PathNameMatches(1, SqlStructureConstants.CASE_THEN)
                                    )
                                )
                            {
                                if (sqlTree.PathNameMatches(0, SqlStructureConstants.CASE_INPUT))
                                    sqlTree.MoveToAncestorContainer(1, SqlStructureConstants.CASE_STATEMENT);
                                else
                                    sqlTree.MoveToAncestorContainer(3, SqlStructureConstants.CASE_STATEMENT);

                                sqlTree.StartNewContainer(SqlStructureConstants.CASE_WHEN, token.Value, SqlStructureConstants.CONTAINER_GENERALCONTENT);
                            }
                            else if ((sqlTree.PathNameMatches(0, SqlStructureConstants.CONTAINER_GENERALCONTENT)
                                    && sqlTree.PathNameMatches(1, SqlStructureConstants.MERGE_CONDITION)
                                    )
                                || sqlTree.PathNameMatches(0, SqlStructureConstants.MERGE_WHEN)
                                )
                            {
                                if (sqlTree.PathNameMatches(1, SqlStructureConstants.MERGE_CONDITION))
                                    sqlTree.MoveToAncestorContainer(2, SqlStructureConstants.MERGE_CLAUSE);
                                else
                                    sqlTree.MoveToAncestorContainer(1, SqlStructureConstants.MERGE_CLAUSE);

                                sqlTree.StartNewContainer(SqlStructureConstants.MERGE_WHEN, token.Value, SqlStructureConstants.CONTAINER_GENERALCONTENT);
                            }
                            else
                                sqlTree.SaveNewElementWithError(SqlStructureConstants.OTHERNODE, token.Value);
                        }
                        else if (significantTokensString.StartsWith("THEN "))
                        {
                            ...
                            if (sqlTree.PathNameMatches(0, SqlStructureConstants.CONTAINER_GENERALCONTENT)
                                && sqlTree.PathNameMatches(1, SqlStructureConstants.CASE_WHEN)
                                )
                            {
                                sqlTree.MoveToAncestorContainer(1, SqlStructureConstants.CASE_WHEN);
                                sqlTree.StartNewContainer(SqlStructureConstants.CASE_THEN, token.Value, SqlStructureConstants.CONTAINER_GENERALCONTENT);
                            }
                            else if (sqlTree.PathNameMatches(0, SqlStructureConstants.CONTAINER_GENERALCONTENT)
                                && sqlTree.PathNameMatches(1, SqlStructureConstants.MERGE_WHEN)
                                )
                            {
                                sqlTree.MoveToAncestorContainer(1, SqlStructureConstants.MERGE_WHEN);
                                sqlTree.StartNewContainer(SqlStructureConstants.MERGE_THEN, token.Value, SqlStructureConstants.MERGE_ACTION);
                                sqlTree.StartNewStatement();
                            }
                            else
                                sqlTree.SaveNewElementWithError(SqlStructureConstants.OTHERNODE, token.Value);
                        ...
                        else if (significantTokensString.StartsWith("OUTPUT "))
                        {
                            ...
                            if (sqlTree.PathNameMatches(0, SqlStructureConstants.SQL_CLAUSE)
                                && sqlTree.PathNameMatches(1, SqlStructureConstants.SQL_STATEMENT)
                        ...
                        else if (significantTokensString.StartsWith("END TRY "))
                        {
                            ...
                            if (sqlTree.PathNameMatches(0, SqlStructureConstants.SQL_CLAUSE)
                                && sqlTree.PathNameMatches(1, SqlStructureConstants.SQL_STATEMENT)
                                && sqlTree.PathNameMatches(2, SqlStructureConstants.CONTAINER_MULTISTATEMENT)
                                && sqlTree.PathNameMatches(3, SqlStructureConstants.TRY_BLOCK)
                                )
                            {
                                //clause.statement.multicontainer.try
                                Node tryBlock = sqlTree.CurrentContainer.Parent.Parent.Parent;
                                Node tryContainerClose = sqlTree.SaveNewElement(SqlStructureConstants.CONTAINER_CLOSE, "", tryBlock);
                        ...
                        else if (significantTokensString.StartsWith("END CATCH "))
                        {
                            ...
                            if (sqlTree.PathNameMatches(0, SqlStructureConstants.SQL_CLAUSE)
                                && sqlTree.PathNameMatches(1, SqlStructureConstants.SQL_STATEMENT)
                                && sqlTree.PathNameMatches(2, SqlStructureConstants.CONTAINER_MULTISTATEMENT)
                                && sqlTree.PathNameMatches(3, SqlStructureConstants.CATCH_BLOCK)
                                )
                            {
                                //clause.statement.multicontainer.catch
                                Node catchBlock = sqlTree.CurrentContainer.Parent.Parent.Parent;
                                Node catchContainerClose = sqlTree.SaveNewElement(SqlStructureConstants.CONTAINER_CLOSE, "", catchBlock);
                        ...
                        else if (significantTokensString.StartsWith("END "))
                        {
                            ...
                            {
                                sqlTree.MoveToAncestorContainer(3, SqlStructureConstants.CASE_STATEMENT);
                                sqlTree.SaveNewElement(SqlStructureConstants.OTHERKEYWORD, token.Value, sqlTree.SaveNewElement(SqlStructureConstants.CONTAINER_CLOSE, ""));
                                sqlTree.MoveToAncestorContainer(1); //unnamed container
                            }
                            else if (sqlTree.PathNameMatches(0, SqlStructureConstants.CONTAINER_GENERALCONTENT)
                                && sqlTree.PathNameMatches(1, SqlStructureConstants.CASE_ELSE)
                                )
                            {
                                sqlTree.MoveToAncestorContainer(2, SqlStructureConstants.CASE_STATEMENT);
                                sqlTree.SaveNewElement(SqlStructureConstants.OTHERKEYWORD, token.Value, sqlTree.SaveNewElement(SqlStructureConstants.CONTAINER_CLOSE, ""));
                                sqlTree.MoveToAncestorContainer(1); //unnamed container
                            }
                            else
                            {
                                ...
                                if (sqlTree.PathNameMatches(0, SqlStructureConstants.SQL_CLAUSE)
                                    && sqlTree.PathNameMatches(1, SqlStructureConstants.SQL_STATEMENT)
                                    && sqlTree.PathNameMatches(2, SqlStructureConstants.CONTAINER_MULTISTATEMENT)
                                    && sqlTree.PathNameMatches(3, SqlStructureConstants.BEGIN_END_BLOCK)
                                    )
                                {
                                    Node beginBlock = sqlTree.CurrentContainer.Parent.Parent.Parent;
                                    Node beginContainerClose = sqlTree.SaveNewElement(SqlStructureConstants.CONTAINER_CLOSE, "", beginBlock);
                                    sqlTree.SaveNewElement(SqlStructureConstants.OTHERKEYWORD, token.Value, beginContainerClose);
                                    sqlTree.CurrentContainer = beginBlock.Parent;
                                }
                                else
                                {
                                    sqlTree.SaveNewElementWithError(SqlStructureConstants.OTHERKEYWORD, token.Value);
                                }
                            }
                        }
                        else if (significantTokensString.StartsWith("GO "))
                        {
                                ...
                                )
                            {
                                ...
                                if (sqlTree.FindValidBatchEnd())
                                {
                                    ...
                                    Node batchSeparator = sqlTree.SaveNewElement(SqlStructureConstants.BATCH_SEPARATOR, "", sqlRoot);
                                    sqlTree.SaveNewElement(SqlStructureConstants.OTHERKEYWORD, token.Value, batchSeparator);
                            ...
                            )
                        {
                            ...
                            {
                                int existingClauseCount = sqlTree.CurrentContainer.Parent != null ? sqlTree.CurrentContainer.Parent.ChildrenByName(SqlStructureConstants.SQL_CLAUSE).Count() : 0;
                        ...
                        else if (_JoinDetector.IsMatch(significantTokensString))
                        {
                            ...
                            sqlTree.CurrentContainer = sqlTree.SaveNewElement(SqlStructureConstants.SELECTIONTARGET, "");
                        }
                        else if (significantTokensString.StartsWith("UNION ALL "))
                        {
                            sqlTree.ConsiderStartingNewClause();
                            ProcessCompoundKeyword(tokenList, sqlTree, sqlTree.SaveNewElement(SqlStructureConstants.SET_OPERATOR_CLAUSE, ""), ref tokenID, significantTokenPositions, 2);
                        }
                        ...
                        {
                            sqlTree.ConsiderStartingNewClause();
                            Node unionClause = sqlTree.SaveNewElement(SqlStructureConstants.SET_OPERATOR_CLAUSE, "");
                            sqlTree.SaveNewElement(SqlStructureConstants.OTHERKEYWORD, token.Value, unionClause);
                        }
                        else if (significantTokensString.StartsWith("WHILE "))
                        {
                            sqlTree.ConsiderStartingNewStatement();
                            Node newWhileLoop = sqlTree.SaveNewElement(SqlStructureConstants.WHILE_LOOP, "");
                            Node whileContainerOpen = sqlTree.SaveNewElement(SqlStructureConstants.CONTAINER_OPEN, "", newWhileLoop);
                            sqlTree.SaveNewElement(SqlStructureConstants.OTHERKEYWORD, token.Value, whileContainerOpen);
                            sqlTree.CurrentContainer = sqlTree.SaveNewElement(SqlStructureConstants.BOOLEAN_EXPRESSION, "", newWhileLoop);
                        }
                        else if (significantTokensString.StartsWith("IF "))
                        {
                            sqlTree.ConsiderStartingNewStatement();
                            sqlTree.StartNewContainer(SqlStructureConstants.IF_STATEMENT, token.Value, SqlStructureConstants.BOOLEAN_EXPRESSION);
                        }
                        else if (significantTokensString.StartsWith("ELSE "))
                        {
                            ...
                            {
                                sqlTree.MoveToAncestorContainer(3, SqlStructureConstants.CASE_STATEMENT);
                                sqlTree.StartNewContainer(SqlStructureConstants.CASE_ELSE, token.Value, SqlStructureConstants.CONTAINER_GENERALCONTENT);
                            }
                            else
                            {
                                ...
                                if (sqlTree.PathNameMatches(0, SqlStructureConstants.SQL_CLAUSE)
                                    && sqlTree.PathNameMatches(1, SqlStructureConstants.SQL_STATEMENT)
                                    && sqlTree.PathNameMatches(2, SqlStructureConstants.CONTAINER_SINGLESTATEMENT)
                                    )
                                {
                                    ...
                                    while (!stopSearching)
                                    {
                                        if (sqlTree.PathNameMatches(currentNode, 1, SqlStructureConstants.IF_STATEMENT))
                                        {
                                            //if this is in an "If", then the "Else" must still be available - yay!
                                            sqlTree.CurrentContainer = currentNode.Parent;
                                            sqlTree.StartNewContainer(SqlStructureConstants.ELSE_CLAUSE, token.Value, SqlStructureConstants.CONTAINER_SINGLESTATEMENT);
                                            sqlTree.StartNewStatement();
                                            stopSearching = true;
                                        }
                                        else if (sqlTree.PathNameMatches(currentNode, 1, SqlStructureConstants.ELSE_CLAUSE))
                                        {
                                        ...
                                        else if (sqlTree.PathNameMatches(currentNode, 1, SqlStructureConstants.WHILE_LOOP))
                                        {
                        ...
                        else if (significantTokensString.StartsWith("SELECT "))
                        {
                            ...
                            if (sqlTree.PathNameMatches(0, SqlStructureConstants.SQL_CLAUSE))
                            {
                                ...
                                foreach (Node clause in sqlTree.CurrentContainer.Parent.ChildrenByName(SqlStructureConstants.SQL_CLAUSE))
                                ...
                                if (firstEntryOfThisClause != null && firstEntryOfThisClause.Name.Equals(SqlStructureConstants.SET_OPERATOR_CLAUSE))
                                    selectShouldntTryToStartNewStatement = true;
                            }
                        ...
                        else if (significantTokensString.StartsWith("UPDATE "))
                        {
                            ...
                            if (!(sqlTree.PathNameMatches(0, SqlStructureConstants.CONTAINER_GENERALCONTENT)
                                    && sqlTree.PathNameMatches(1, SqlStructureConstants.CURSOR_FOR_OPTIONS)
                                    )
                                )
                            {
                                sqlTree.ConsiderStartingNewStatement();
                        ...
                        {
                            if (sqlTree.PathNameMatches(0, SqlStructureConstants.CONTAINER_GENERALCONTENT)
                                && sqlTree.PathNameMatches(1, SqlStructureConstants.PERMISSIONS_TARGET)
                                )
                            {
                                sqlTree.MoveToAncestorContainer(2, SqlStructureConstants.PERMISSIONS_BLOCK);
                                sqlTree.StartNewContainer(SqlStructureConstants.PERMISSIONS_RECIPIENT, token.Value, SqlStructureConstants.CONTAINER_GENERALCONTENT);
                            }
                            else
                            {
                        ...
                        else if (significantTokensString.StartsWith("FROM "))
                        {
                            ...
                            && sqlTree.PathNameMatches(0, SqlStructureConstants.CONTAINER_GENERALCONTENT)
                            && sqlTree.PathNameMatches(1, SqlStructureConstants.PERMISSIONS_RECIPIENT)
                            )
                        {
                            sqlTree.MoveToAncestorContainer(2, SqlStructureConstants.PERMISSIONS_BLOCK);
                            sqlTree.CurrentContainer = sqlTree.SaveNewElement(SqlStructureConstants.CONTAINER_GENERALCONTENT, "", sqlTree.SaveNewElement(SqlStructureConstants.DDL_WITH_CLAUSE, ""));
                            sqlTree.SaveNewElement(SqlStructureConstants.OTHERKEYWORD, token.Value);
                        }
                        else if (significantTokensString.StartsWith("SET "))
                        {
                            ...
                            if (!(
                                    ...
                                    && firstNonCommentSibling2.Name.Equals(SqlStructureConstants.OTHERKEYWORD)
                        ...
                        {
                            sqlTree.CurrentContainer = sqlTree.SaveNewElement(SqlStructureConstants.BETWEEN_CONDITION, "");
                            sqlTree.SaveNewElement(SqlStructureConstants.OTHERKEYWORD, token.Value, sqlTree.SaveNewElement(SqlStructureConstants.CONTAINER_OPEN, ""));
                            sqlTree.CurrentContainer = sqlTree.SaveNewElement(SqlStructureConstants.BETWEEN_LOWERBOUND, "");
                        }
                        else if (significantTokensString.StartsWith("AND "))
                        {
                            if (sqlTree.PathNameMatches(0, SqlStructureConstants.BETWEEN_LOWERBOUND))
                            {
                                sqlTree.MoveToAncestorContainer(1, SqlStructureConstants.BETWEEN_CONDITION);
                                sqlTree.SaveNewElement(SqlStructureConstants.OTHERKEYWORD, token.Value, sqlTree.SaveNewElement(SqlStructureConstants.CONTAINER_CLOSE, ""));
                                sqlTree.CurrentContainer = sqlTree.SaveNewElement(SqlStructureConstants.BETWEEN_UPPERBOUND, "");
                            }
                            else
                            {
                                sqlTree.EscapeAnyBetweenConditions();
                                sqlTree.SaveNewElement(SqlStructureConstants.OTHERKEYWORD, token.Value, sqlTree.SaveNewElement(SqlStructureConstants.AND_OPERATOR, ""));
                            }
                        }
                        else if (significantTokensString.StartsWith("OR "))
                        {
                            sqlTree.EscapeAnyBetweenConditions();
                            sqlTree.SaveNewElement(SqlStructureConstants.OTHERKEYWORD, token.Value, sqlTree.SaveNewElement(SqlStructureConstants.OR_OPERATOR, ""));
                        }
                        else if (significantTokensString.StartsWith("WITH "))
                        {
                            ...
                            {
                                sqlTree.CurrentContainer = sqlTree.SaveNewElement(SqlStructureConstants.CTE_WITH_CLAUSE, "");
                                sqlTree.SaveNewElement(SqlStructureConstants.OTHERKEYWORD, token.Value, sqlTree.SaveNewElement(SqlStructureConstants.CONTAINER_OPEN, ""));
                                sqlTree.CurrentContainer = sqlTree.SaveNewElement(SqlStructureConstants.CTE_ALIAS, "");
                            }
                            else if (sqlTree.PathNameMatches(0, SqlStructureConstants.CONTAINER_GENERALCONTENT)
                                && sqlTree.PathNameMatches(1, SqlStructureConstants.PERMISSIONS_RECIPIENT)
                                )
                            {
                                sqlTree.MoveToAncestorContainer(2, SqlStructureConstants.PERMISSIONS_BLOCK);
                                sqlTree.StartNewContainer(SqlStructureConstants.DDL_WITH_CLAUSE, token.Value, SqlStructureConstants.CONTAINER_GENERALCONTENT);
                            }
                            else if (sqlTree.PathNameMatches(0, SqlStructureConstants.DDL_PROCEDURAL_BLOCK)
                                || sqlTree.PathNameMatches(0, SqlStructureConstants.DDL_OTHER_BLOCK)
                                )
                            ...
                            else if (sqlTree.PathNameMatches(0, SqlStructureConstants.SELECTIONTARGET))
                            {
                                sqlTree.SaveNewElement(SqlStructureConstants.OTHERKEYWORD, token.Value);
                            }
                            else
                        ...
                        {
                            sqlTree.ConsiderStartingNewStatement();
                            sqlTree.SaveNewElement(SqlStructureConstants.LABEL, token.Value + tokenList[tokenID + 1].Value);
                            tokenID++;
                        }
                        else
                        {
                            ...
                            string newNodeName = SqlStructureConstants.OTHERNODE;
                            ...
                            if (KeywordList.TryGetValue(token.Value.ToUpperInvariant(), out matchedKeywordType))
                            {
                                switch (matchedKeywordType)
                                {
                                    case KeywordType.OperatorKeyword:
                                        newNodeName = SqlStructureConstants.ALPHAOPERATOR;
                                        break;
                                    case KeywordType.FunctionKeyword:
                                        newNodeName = SqlStructureConstants.FUNCTION_KEYWORD;
                                        break;
                                    case KeywordType.DataTypeKeyword:
                                        newNodeName = SqlStructureConstants.DATATYPE_KEYWORD;
                                        break;
                                    case KeywordType.OtherKeyword:
                                        sqlTree.EscapeAnySelectionTarget();
                                        newNodeName = SqlStructureConstants.OTHERKEYWORD;
                                        break;
                                    default:
                    ...
                    case SqlTokenType.Semicolon:
                        sqlTree.SaveNewElement(SqlStructureConstants.SEMICOLON, token.Value);
                    ...
                    case SqlTokenType.Colon:
                        ...
                        {
                            sqlTree.SaveNewElement(SqlStructureConstants.SCOPERESOLUTIONOPERATOR, token.Value + tokenList[tokenID + 1].Value);
                            tokenID++;
                        }
                        else if (tokenList.Count > tokenID + 1
                            ...
                            )
                        {
                            ...
                            sqlTree.SaveNewElement(SqlStructureConstants.OTHERNODE, token.Value + tokenList[tokenID + 1].Value);
                            tokenID++;
                        }
                        else
                        {
                            sqlTree.SaveNewElementWithError(SqlStructureConstants.OTHEROPERATOR, token.Value);
                        }
                        break;

                    case SqlTokenType.Comma:
                        bool isCTESplitter = (sqlTree.PathNameMatches(0, SqlStructureConstants.CONTAINER_GENERALCONTENT)
                            && sqlTree.PathNameMatches(1, SqlStructureConstants.CTE_WITH_CLAUSE)
                            );
                    ...
                    case SqlTokenType.EqualsSign:
                        sqlTree.SaveNewElement(SqlStructureConstants.EQUALSSIGN, token.Value);
                        if (sqlTree.PathNameMatches(0, SqlStructureConstants.DDL_DECLARE_BLOCK))
                            sqlTree.CurrentContainer = sqlTree.SaveNewElement(SqlStructureConstants.CONTAINER_GENERALCONTENT, "");
                        break;
        ...
        private static bool ContentStartsWithKeyword(Node providedContainer, string contentToMatch)
        {
            ...
            if (firstEntryOfProvidedContainer != null
                && firstEntryOfProvidedContainer.Name.Equals(SqlStructureConstants.OTHERKEYWORD)
                && firstEntryOfProvidedContainer.TextValue != null
                )
                keywordUpperValue = firstEntryOfProvidedContainer.TextValue.ToUpperInvariant();

            if (firstEntryOfProvidedContainer != null
                && firstEntryOfProvidedContainer.Name.Equals(SqlStructureConstants.COMPOUNDKEYWORD)
                )
        ...
        {
            Node compoundKeyword = sqlTree.SaveNewElement(SqlStructureConstants.COMPOUNDKEYWORD, "", targetContainer);
            ...
            AppendNodesWithMapping(sqlTree, tokenList.GetRangeByIndex(significantTokenPositions[0], significantTokenPositions[keywordCount - 1]), SqlStructureConstants.OTHERKEYWORD, compoundKeyword);
        ...
        private string GetEquivalentSqlNodeName(SqlTokenType tokenType)
        {
            ...
            {
                case SqlTokenType.WhiteSpace:
                    return SqlStructureConstants.WHITESPACE;
                case SqlTokenType.SingleLineComment:
                    return SqlStructureConstants.COMMENT_SINGLELINE;
                case SqlTokenType.SingleLineCommentCStyle:
                    return SqlStructureConstants.COMMENT_SINGLELINE_CSTYLE;
                case SqlTokenType.MultiLineComment:
                    return SqlStructureConstants.COMMENT_MULTILINE;
                case SqlTokenType.BracketQuotedName:
                    return SqlStructureConstants.BRACKET_QUOTED_NAME;
                case SqlTokenType.Asterisk:
                    return SqlStructureConstants.ASTERISK;
                case SqlTokenType.EqualsSign:
                    return SqlStructureConstants.EQUALSSIGN;
                case SqlTokenType.Comma:
                    return SqlStructureConstants.COMMA;
                case SqlTokenType.Period:
                    return SqlStructureConstants.PERIOD;
                case SqlTokenType.NationalString:
                    return SqlStructureConstants.NSTRING;
                case SqlTokenType.String:
                    return SqlStructureConstants.STRING;
                case SqlTokenType.QuotedString:
                    return SqlStructureConstants.QUOTED_STRING;
                case SqlTokenType.OtherOperator:
                    return SqlStructureConstants.OTHEROPERATOR;
                case SqlTokenType.Number:
                    return SqlStructureConstants.NUMBER_VALUE;
                case SqlTokenType.MonetaryValue:
                    return SqlStructureConstants.MONETARY_VALUE;
                case SqlTokenType.BinaryValue:
                    return SqlStructureConstants.BINARY_VALUE;
                case SqlTokenType.PseudoName:
                    return SqlStructureConstants.PSEUDONAME;
                default:
        ...
        private bool IsLatestTokenADDLDetailValue(ParseTree sqlTree)
        {
            ...
            if (latestContentNode != null
                && (latestContentNode.Name.Equals(SqlStructureConstants.OTHERKEYWORD)
                    || latestContentNode.Name.Equals(SqlStructureConstants.DATATYPE_KEYWORD)
                    || latestContentNode.Name.Equals(SqlStructureConstants.COMPOUNDKEYWORD)
                    ))
            {
                string uppercaseText = null;
                if (latestContentNode.Name.Equals(SqlStructureConstants.COMPOUNDKEYWORD))
        ...
        private bool IsLatestTokenAComma(ParseTree sqlTree)
        {
            ...
            return latestContent != null && latestContent.Name.Equals(SqlStructureConstants.COMMA);
        }
        ...
        private bool IsLatestTokenAMiscName(ParseTree sqlTree)
        {
            ...
            if (latestContent != null)
            {
                ...
                if (latestContent.Name.Equals(SqlStructureConstants.BRACKET_QUOTED_NAME))
                    return true;

                if ((latestContent.Name.Equals(SqlStructureConstants.OTHERNODE)
                        || latestContent.Name.Equals(SqlStructureConstants.FUNCTION_KEYWORD)
                        )
